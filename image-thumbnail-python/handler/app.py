...
def handler_s3_thumbnail_generator(event, context):
    print("Event::", event)
...
def handler_s3_get_thumbnail_urls(event, context):
    table = dynamodb.Table(dbtable)
    response = table.scan()
    data = response['Items']
    #paginate 
...
def s3_delete_item(event, context):
    ...
    response = {
        'statusCode': 500,
        "body": f"An error occured while deleting image {item_id}"
    }
    table = dynamodb.Table(dbtable)
